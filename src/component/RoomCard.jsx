...
export const RoomCard = ({ room, type = "user", onEdit, onDelete, onClick }) => {
  ...
  return (
    <div 
      ...
      className={cn(
        "terminal-card group block transition-all duration-300 relative h-full flex flex-col justify-between cursor-pointer",
        "hover:border-primary/50 hover:shadow-[0_0_30px_rgba(0,255,128,0.2)]",
        (!isActive && !isAdmin) ? "opacity-50 grayscale" : "" 
      )}
    >
        {/* HEADER */}
        <div className="p-6 border-b border-border">
          <div className="flex items-start justify-between mb-4">
             <div className="flex items-center gap-3">
                 ...
                 <div>
                   <h3 className="font-display text-lg group-hover:text-primary transition-colors uppercase tracking-tight">
                       {room.name}
                   </h3>
                 </div>
             </div>
             <span className={cn("text-[10px] px-2 py-1 rounded border font-mono uppercase", difficultyColors[room.difficulty] || difficultyColors.Beginner)}>
                 {room.difficulty}
             </span>
          </div>
          ...
          <div className="flex items-center justify-between text-sm mb-3 font-mono">
             <div className="flex items-center gap-4">
                 <span className="flex items-center gap-1 text-muted-foreground group-hover:text-primary">
                    <Target className="w-4 h-4" /> {challengeCount} Tasks
                 </span>
          ...
          {isAdmin && (
             <div className="flex gap-2 mt-2 pt-2 border-t border-border/50">
                 <button 
                    ...
                    className="flex-1 py-1 text-[10px] font-bold border border-primary/30 text-primary hover:bg-primary/10 transition-colors"
                 >
                    EDIT
                 </button>
                 <button 
                    onClick={(e) => { e.stopPropagation(); onDelete(room._id); }} 
                    className="flex-1 py-1 text-[10px] font-bold border border-destructive/30 text-destructive hover:bg-destructive/10 transition-colors"
                 >
                    DELETE
                 </button>
             </div>
