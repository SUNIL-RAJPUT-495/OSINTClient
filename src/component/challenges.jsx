...
export const ChallengeView = () => {
  ...
  // FIX 1: Variable ka naam sahi kiya (Confusion hatane ke liye)
  ...
  // FIX 2: Question change hone par input aur error reset karo
  useEffect(() => {
    setFlagInput("");
  ...
  const handleFlagSubmit = async (e) => {
    ...
    // FIX 3: MOBILE KEYBOARD CLOSE KARO (Sabse Important)
    ...
    setIsWrong(false); // Submit karte hi purana error hatao
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (!response.data.correct) {
            setIsWrong(true); // Red Alert dikhao
      ...
      // Mobile par error dekhne ke liye toast use karo
      toast.error(error.response?.data?.message || "Submission Failed. Check Internet.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen matrix-bg text-foreground">
      ...
      {/* FIX 4: Padding badhayi taaki keyboard content ko chupaye nahi */}
          ...
          <div className="lg:col-span-3 space-y-6">
            {currentChallenge ? (
              <div className={cn("terminal-card p-6 border-primary/50 relative overflow-hidden transition-all duration-500", isCurrentSolved && "border-green-500/30 bg-green-500/[0.01]")}>
                ...
                {/* --- FIX 5: Sahi Logic for Error Display --- */}
                {isWrong && (
                  <div className="mb-4 p-4 bg-red-500/10 border border-red-500/30 rounded flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                    ...
                    <span className="text-sm font-mono text-red-400">ALERT: FLAG_INCORRECT. NO_XP_AWARDED.</span>
                  </div>
                )}
                        ...
                        <div className="relative flex-1">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                                ...
                                setIsWrong(false); // Typing karte waqt error hatao
                            }}
                            required
                          />
                        </div>
